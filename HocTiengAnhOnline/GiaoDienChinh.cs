...
using HocTiengAnhOnline.Properties;
...
namespace HocTiengAnhOnline
{
    public partial class GiaoDienChinh : Form
    {
        ...
        string tkNguoiDung;
        string mkNguoiDung;
        public GiaoDienChinh(string tkDaLuu, string mkDaLuu)
        {
            con = new ConnectSQL();
            conn = con.connection();
            InitializeComponent();
        ...
        private void ShowCauHoi()
        {
            ...
            if(CauHoiHienTai >= DanhSachCauHoi.Rows.Count)
            {
                ...
                LuuKetQua(txtMaDTinLamDT.Text.Trim(), tkNguoiDung, ((float)SoCauDung / DanhSachCauHoi.Rows.Count * 10));
                return;
            }
        ...
        private void LuuKetQua(string maDT, string maHV, float diem)
        {
            string sql = string.Format("insert into Diem(MaDT, MaHV, Diem) values('{0}','{1}','{2}')", maDT, maHV, diem);
        ...
        // Ho so
        private void btnEditinHoSo_Click(object sender, EventArgs e)
        {
            choPhepChinhSua();
        }
        ...
        {
            txtTenHVinHoSo.Enabled= true;
            cbGioiTinhinHoSo.Enabled= true;
            txtDiaChiinHoSo.Enabled= true;
            dtpinHoSo.Enabled= true;

            btnEditinHoSo.Enabled= false;
            btnLuuinHoSo.Enabled= true;
        }

        private void koChoPhepChinhSua()
        {
            txtTenHVinHoSo.Enabled = false;
            cbGioiTinhinHoSo.Enabled = false;
            txtDiaChiinHoSo.Enabled = false;
            dtpinHoSo.Enabled = false;

            btnEditinHoSo.Enabled = true;
            btnLuuinHoSo.Enabled = false;
        }

        private void menuTTNguoiDung_Click(object sender, EventArgs e)
        {
            koChoPhepChinhSua();            
            getThongTinNguoiDung();            

            tabControl1.SelectedTab = tabPageHoSoSV;
        }

        private void btnLuuinHoSo_Click(object sender, EventArgs e)
        {
            koChoPhepChinhSua();

            string sql = string.Format("update HocVien set HoTen=N'{0}', NgaySinh='{1}', GioiTinh= N'{2}', DiaChi= N'{3}' where MaHV='{4}'", txtTenHVinHoSo.Text, dtpinHoSo.Value, cbGioiTinhinHoSo.SelectedItem, txtDiaChiinHoSo.Text, txtMaHVinHoSo.Text);
            ct.ExecuteNonQuery(sql);
        ...
        {
            txtMaHVinHoSo.Text = tkNguoiDung;
            string sql = string.Format("select HoTen, GioiTinh, NgaySinh, DiaChi from HocVien where MaHV = '{0}'", tkNguoiDung);
            DataTable dt = ct.ExecuteQuery(sql);
            ...
            txtTenHVinHoSo.Text = dr["HoTen"].ToString();
            cbGioiTinhinHoSo.SelectedItem = dr["GioiTinh"].ToString();
            if (dr["GioiTinh"].ToString().Equals("Nam"))
            {
                pbAvatar.Image = Resources.AvaNam;
            }
            if (dr["GioiTinh"].ToString().Equals("Nữ"))
            {
                pbAvatar.Image = Resources.AvaNu;
            }


            dtpinHoSo.Value = Convert.ToDateTime(dr["NgaySinh"]);
            txtDiaChiinHoSo.Text = dr["DiaChi"].ToString();
        }
        ...
        private void menuItemLSLamBai_Click(object sender, EventArgs e)
        {
            tabControl1.SelectedTab = tabPageLichSuLamBai;
            loadDataTableLSLamBai();

        }

        private void loadDataTableLSLamBai()
        {
            string sql = "select TenKH, TenDT, Diem from KhoaHoc, DeThi, Diem where DeThi.MaKH = KhoaHoc.MaKH and DeThi.MaDT = Diem.MaDT";
            dgvLichSuLamBai.DataSource = ct.ExecuteQuery(sql);
            dgvLichSuLamBai.Columns["TenKH"].Width = 200;
            dgvLichSuLamBai.Columns["TenDT"].Width = 200;
            dgvLichSuLamBai.Columns["Diem"].Width = 100;
        }
    }
}
